...
*/
void list::add_line_to_word_map(std::string line)
{
	...
	{
		auto search = std::find(std::begin(dictionary), 
			std::end(dictionary), *beg);
...
*/
void list::send_word_count_to_file(std::ofstream &out_stream)
{
	std::map<std::string, unsigned>::iterator word_map_it;

	if (out_stream.good())
	{
		for (auto dict_it = dictionary.begin(); 
			dict_it != dictionary.end(); ++dict_it)
		{
			...
			else
			{
				...
				for (auto dict_it_cmp = dictionary.begin();
					dict_it_cmp != dictionary.end(); ++dict_it_cmp)
				{
